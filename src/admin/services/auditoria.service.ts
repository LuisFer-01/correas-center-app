...
async function getAuditoriaLogsForExport(filters: AuditoriaFilters = {}): Promise<AuditoriaLog[]> {
  let query = supabase
    .from('auditoria')
    .select(`*, usuario:perfiles ( id, nombre_completo, email )`)
    .order('creado_en', { ascending: false })

  if (filters.usuario_id) query = query.eq('usuario_id', filters.usuario_id)
  if (filters.accion && filters.accion !== 'all') query = query.eq('accion', filters.accion)
  if (filters.tabla_afectada && filters.tabla_afectada !== 'all') query = query.eq('tabla_afectada', filters.tabla_afectada)
  if (filters.fecha_inicio) query = query.gte('creado_en', filters.fecha_inicio)
  if (filters.fecha_fin) query = query.lte('creado_en', filters.fecha_fin)
...
export async function getAuditoriaLogs(filters: AuditoriaFilters = {}): Promise<{ logs: AuditoriaLog[]; total: number }> {
  const limit = filters.limit || 50
  const offset = filters.offset || 0

  // 1. Obtener el total
  let countQuery = supabase.from('auditoria').select('*', { count: 'exact', head: true })
  ...
  const logs = await getAuditoriaLogsForExport({ ...filters, limit, offset })
  
  return {
    logs: logs.slice(0, limit), // Ya viene ordenado y filtrado, cortamos al límite
    total: count || 0,
  }
}
...
export async function getUsuariosConActividad() {
  const { data, error } = await supabase
    .from('auditoria')
    .select(`usuario_id, usuario:perfiles ( id, nombre_completo, email )`)
...
export async function getTablasAfectadas() {
  const { data, error } = await supabase.from('auditoria').select('tabla_afectada').order('creado_en', { ascending: false })
  if (error) throw error
  const uniqueTables = new Set(data?.map((item: any) => item.tabla_afectada) || [])
  return Array.from(uniqueTables).sort()
}

export async function getAccionesDisponibles() {
  const { data, error } = await supabase.from('auditoria').select('accion').order('creado_en', { ascending: false })
  if (error) throw error
  const uniqueActions = new Set(data?.map((item: any) => item.accion) || [])
...
function transformarLogsParaExportar(logs: AuditoriaLog[]) {
  return logs.map((log) => ({
    'ID': log.id,
    'Fecha/Hora': new Date(log.creado_en).toLocaleString('es-BO', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    'Acción': log.accion,
    ...
    'Metadata': log.metadata && Object.keys(log.metadata).length > 0 ? JSON.stringify(log.metadata) : '',
  }))
}
...
export async function exportarAuditoriaExcel(filters: AuditoriaFilters = {}): Promise<Blob> {
  ...
  worksheet['!cols'] = [
    { wch: 6 }, { wch: 20 }, { wch: 12 }, { wch: 25 }, { wch: 30 },
    { wch: 20 }, { wch: 15 }, { wch: 15 }, { wch: 40 }, { wch: 50 }, { wch: 50 }, { wch: 40 },
  ]
