import type { CreatePasoWizardDTO, PasoWizard, UpdatePasoWizardDTO } from '@/admin/types/pasos-wizard'
import { supabase } from '@/lib/supabase'

export async function getPasosWizard(includeDeleted: boolean = false): Promise<PasoWizard[]> {
  let query = supabase
    .from('pasos_wizard')
    .select(`
      *,
  ...
  query = query.order('orden', { ascending: true }).order('creado_en', { ascending: true })
...
export async function getNextOrdenPasoWizard(): Promise<number> {
  const { data, error } = await supabase
    .from('pasos_wizard')
    .select('orden')
