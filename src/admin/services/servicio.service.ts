import type { CreateServicioDTO, Servicio, UpdateServicioDTO } from '@/admin/types/servicio'
...
export async function getServicios(includeDeleted: boolean = false): Promise<Servicio[]> {
  ...
  // Cargar asignaciones de industrias por separado
  if (servicios.length > 0) {
    ...
    const { data: asignacionesData, error: asignacionesError } = await supabase
      ...
      .select(`
        *,
        industria:industrias(id, nombre, slug)
      `)
