...
// Mapeo de iconos de Font Awesome a Lucide o emojis
...
export const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    // TODO: Implementar suscripción a newsletter con Supabase
    // await supabase.from('suscriptores').insert({ email, nombre, empresa_id: globals?.empresa?.id })
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 mb-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-3 mb-6">
              {globals?.footer_redes_sociales && globals.footer_redes_sociales.map((red) => (
                <a
                  key={red.id}
                  href={red.url || '#'}
                  target="_blank"
                  ...
                  title={red.titulo || ''}
                >
                  <span className="text-lg">{iconMap[red.icono || ''] || '🔗'}</span>
                </a>
              ))}
            </div>
          ...
          <div className="lg:col-span-1">
            ...
            <ul className="space-y-2 text-sm">
              {globals?.footer_productos && globals.footer_productos.slice(0, 8).map((item) => (
                <li key={item.id}>
                  {item.producto && (
                    <Link
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm mb-6">
              {globals?.footer_industrias && globals.footer_industrias.slice(0, 6).map((item) => (
                <li key={item.id}>
                  {item.industria && (
                    <Link
                      to={`/applications/${item.industria.slug}`}
                      className="text-gray-400 hover:text-[#EA0A2A] transition-colors hover:translate-x-1 inline-block"
                    >
            ...
            <ul className="space-y-2 text-sm">
              {globals?.footer_servicios && globals.footer_servicios.slice(0, 4).map((item) => (
                <li key={item.id}>
                  {item.servicio && (
                    <Link
                      to={`/services/${item.servicio.nombre.toLowerCase().replace(/\s+/g, '-')}`}
                      className="text-gray-400 hover:text-[#EA0A2A] transition-colors hover:translate-x-1 inline-block"
                    >
          ...
          <div>
            ...
            <div className="space-y-4">
              {globals?.sucursales && globals.sucursales.slice(0, 3).map((sucursal) => (
                <div key={sucursal.id} className="border-l-2 border-[#EA0A2A]/30 pl-3 hover:border-[#EA0A2A] transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6 text-sm text-gray-400">
              <Link to="/privacy" className="hover:text-[#EA0A2A] transition-colors">Política de Privacidad</Link>
              <Link to="/terms" className="hover:text-[#EA0A2A] transition-colors">Términos y Condiciones</Link>
              <Link to="/branches" className="hover:text-[#EA0A2A] transition-colors">Mapa del Sitio</Link>
            </div>
          </div>
        </div>
      </div>
